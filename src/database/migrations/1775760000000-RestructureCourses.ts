...
export class RestructureCourses1775760000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Drop old FK columns and qualification
    await queryRunner.query(`
            ALTER TABLE "courses"
    ...
    // Add university_name as plain string
    await queryRunner.query(`
            ALTER TABLE "courses"
            ADD COLUMN IF NOT EXISTS "university_name" character varying NOT NULL DEFAULT ''
        `);

    // Rename columns safely by searching for the column name case-insensitively
    ...
    for (const col of columnsToRename) {
      // First check if the target column already exists (e.g. migration already ran or table was created with it)
      ...
      if (hasTargetColumn.length === 0) {
        // Target doesn't exist, search for any variation of the old name
        ...
        if (existingCols.length > 0) {
          ...
          await queryRunner.query(
            `ALTER TABLE "courses" RENAME COLUMN "${actualName}" TO "${col.target}"`,
          );
        }
      }
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "courses" RENAME COLUMN "is_active" TO "isActive"`,
    );
    await queryRunner.query(
      `ALTER TABLE "courses" RENAME COLUMN "is_regional" TO "isRegionalPointsEligible"`,
    );
    await queryRunner.query(
      `ALTER TABLE "courses" RENAME COLUMN "annual_fees" TO "annualFees"`,
    );
    await queryRunner.query(
      `ALTER TABLE "courses" RENAME COLUMN "anzsco_title" TO "anzscoTitle"`,
    );
    await queryRunner.query(
      `ALTER TABLE "courses" RENAME COLUMN "anzsco_code" TO "anzscoCode"`,
    );
    await queryRunner.query(
      `ALTER TABLE "courses" RENAME COLUMN "course_title" TO "courseTitle"`,
    );

    // Drop university_name
    await queryRunner.query(
      `ALTER TABLE "courses" DROP COLUMN IF EXISTS "university_name"`,
    );

    // Re-add original FK columns
    await queryRunner.query(`
            ALTER TABLE "courses"
