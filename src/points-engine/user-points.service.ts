...
export class UserPointsService {
  ...
  async getHistory(
    userId: string,
    limit: number = 10,
  ): Promise<{ timestamp: string; totalPoints: number; breakdown: Record<string, any> }[]> {
  ...
  async getOccupationPoints(
    anzscoCode: string,
  ): Promise<{ occupation: string; basePoints: number; modifiers: Record<string, number> }> {
    return {
      occupation: anzscoCode,
      basePoints: 60,
      modifiers: {
        state_nomination: 5,
        regional: 15,
  ...
  async getEligibilityRanges(): Promise<
    { visaType: string; minimumPoints: number; averagePoints: number; percentilePoints: Record<string, number> }[]
  > {
    return [
      {
        visaType: '189',
