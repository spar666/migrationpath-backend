...
export class PaymentsController {
  ...
  @ApiOperation({
    summary:
      'Create a Stripe Checkout session for the consultation fee (public)',
  })
  ...
  /**
   * Runs the reconciliation sweep now (admin only).
   *
   * The sweep normally ticks hourly, but two situations want it on demand: a
   * suspected webhook outage, where every hour of waiting is another customer
   * charged without a confirmed booking, and a deployment that leaves the
   * scheduled sweep switched off. It is safe to call repeatedly — overlapping
   * runs are skipped, and confirmation goes through the same idempotent path
   * the webhook uses.
   */
  @Post('reconcile')
  @ApiBearerAuth()
  ...
  @Roles('admin')
  @ApiOperation({
    summary: 'Recover payments whose webhook never arrived (admin only)',
  })
  reconcile() {
