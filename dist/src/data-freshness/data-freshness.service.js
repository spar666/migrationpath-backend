"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DataFreshnessService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const data_source_meta_entity_1 = require("./entities/data-source-meta.entity");
const DAY_MS = 24 * 60 * 60 * 1000;
let DataFreshnessService = class DataFreshnessService {
    repo;
    constructor(repo) {
        this.repo = repo;
    }
    computeStatus(row) {
        if (!row.lastVerifiedAt)
            return { status: 'stale', daysSinceVerified: null };
        const days = Math.floor((Date.now() - new Date(row.lastVerifiedAt).getTime()) / DAY_MS);
        const interval = row.reviewIntervalDays || 90;
        let status;
        if (days > interval)
            status = 'stale';
        else if (days > interval * 0.75)
            status = 'review';
        else
            status = 'current';
        return { status, daysSinceVerified: days };
    }
    async findAll() {
        const rows = await this.repo.find({ order: { label: 'ASC' } });
        return rows.map((r) => {
            const { status, daysSinceVerified } = this.computeStatus(r);
            return {
                domain: r.domain,
                label: r.label,
                adminRoute: r.adminRoute,
                status,
                lastVerifiedAt: r.lastVerifiedAt,
                reviewIntervalDays: r.reviewIntervalDays,
                daysSinceVerified,
                sourceUrl: r.sourceUrl,
                notes: r.notes,
            };
        });
    }
    async markVerified(domain) {
        const row = await this.repo.findOne({ where: { domain } });
        if (!row)
            throw new common_1.NotFoundException(`Domain "${domain}" not found`);
        row.lastVerifiedAt = new Date();
        await this.repo.save(row);
        const { status, daysSinceVerified } = this.computeStatus(row);
        return {
            domain: row.domain,
            label: row.label,
            adminRoute: row.adminRoute,
            status,
            lastVerifiedAt: row.lastVerifiedAt,
            reviewIntervalDays: row.reviewIntervalDays,
            daysSinceVerified,
            sourceUrl: row.sourceUrl,
            notes: row.notes,
        };
    }
    async update(domain, dto) {
        const row = await this.repo.findOne({ where: { domain } });
        if (!row)
            throw new common_1.NotFoundException(`Domain "${domain}" not found`);
        Object.assign(row, dto);
        return this.repo.save(row);
    }
};
exports.DataFreshnessService = DataFreshnessService;
exports.DataFreshnessService = DataFreshnessService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(data_source_meta_entity_1.DataSourceMeta)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], DataFreshnessService);
//# sourceMappingURL=data-freshness.service.js.map